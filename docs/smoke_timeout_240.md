...
```json
{
  ...
  "result": {
    "question": "请用三条要点总结本地RAG流程。",
    "answer": "本地RAG桌面流程可以总结为以下三条要点：\n\n• 健康检查（doctor）：在开始使用本地RAG前进行健康检查，以确保系统的正常运作。\n• 配置知识目录：配置好知识目录，准备就绪后才能发起查询。\n• 重建索引（index）和发起查询（ask）：如果数据变更了，建议重建索引以保证准确性，然后可以进行发起查询。\n\n注意本地RAG的流程来源为 [D:\\code\\md-rag-ollama-local\\.merged_knowledge\\0_2c7382f4d2_flow.md]。",
    "sources": [
