...
```json
{
  ...
  "result": {
    "question": "请总结本地RAG执行流程，并说明知识库加载与卸载能力。",
    "answer": "根据提供的上下文，我可以总结如下：\n\n**本地RAG执行流程**\n\n* 健康检查（doctor）\n* 配置知识目录\n* 重建索引（index）\n* 发起查询（ask）\n\n**知识库加载与卸载能力**\n\n* 可添加多个知识目录\n* 可按目录移除\n* 每次变更后建议重建索引\n\n请注意，上述信息基于提供的上下文文件：`data_ops.md`、`flow.md`和`model.md`。如果您需要更多信息，请提供相关的上下文或来源。",
    "sources": [
