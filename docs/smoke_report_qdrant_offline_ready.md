...
```json
{
  ...
  "result": {
    "question": "请总结本地RAG执行流程，并说明知识库加载与卸载能力。",
    "answer": "根据提供的上下文信息，以下是总结本地RAG执行流程和知识库加载卸载能力：\n\n**数据加卸载说明**\n\n* 数据加卸载能力：\n\t+ 可添加多个知识目录\n\t+ 可按目录移除\n\t+ 每次变更后建议重建索引\n\n**本地RAG桌面流程**\n\n* 桌面端标准流程：\n\t1. 健康检查（doctor）\n\t2. 配置知识目录\n\t3. 重建索引（index）\n\t4. 发起查询（ask）\n\n注意：上下文中没有提供关于知识库加载和卸载的具体步骤信息，因此根据“数据加卸载说明”部分，我们可以知道有添加多个知识目录和按目录移除的能力，但具体的加载卸载流程尚未明确。\n\n如果需要进一步了解请提供相关上下文信息。",
    "sources": [
