...
```json
{
  ...
  "result": {
    "question": "请总结本地RAG执行流程，并说明知识库加载与卸载能力。",
    "answer": "根据提供的上下文，我可以总结出以下关于本地RAG执行流程和知识库加载卸载能力的信息：\n\n**本地RAG执行流程：**\n\n* 健康检查（doctor）\n* 配置知识目录\n* 重建索引（index）\n* 发起查询（ask）\n\n**知识库加载卸载能力：**\n\n* 可添加多个知识目录\n* 可按目录移除\n* 每次变更后建议重建索引\n\n具体的流程和操作请参考上述文档：`data_ops.md` 和 `flow.md`。",
    "sources": [
