...
```json
{
  ...
  "result": {
    "question": "请总结本地RAG执行流程，并说明知识库加载与卸载能力。",
    "answer": "基于提供的上下文，我可以总结出本地RAG执行流程如下：\n\n**数据加卸载能力**\n\n* 可添加多个知识目录\n* 可按目录移除\n* 每次变更后建议重建索引\n\n**桌面端标准流程**\n\n1. 健康检查（doctor）\n2. 配置知识目录\n3. 重建索引（index）\n4. 发起查询（ask）\n\n关于知识库加载与卸载能力，根据上下文，RAG支持如下功能：\n\n* 可以添加多个知识目录\n* 可按目录移除\n\n这意味着RAG可以轻松地在本地管理和更新知识库。",
    "sources": [
