...
```json
{
  ...
  "result": {
    "question": "请总结本地RAG执行流程，并说明知识库加载与卸载能力。",
    "answer": "根据提供的上下文信息，总结本地RAG执行流程如下：\n\n**数据加卸载说明**\n\n* 数据加卸载能力：\n\t+ 可添加多个知识目录\n\t+ 可按目录移除\n\t+ 每次变更后建议重建索引\n\n**本地RAG桌面流程**\n\n* 桌面端标准流程：\n\t1. 健康检查（doctor）\n\t2. 配置知识目录\n\t3. 重建索引（index）\n\t4. 发起查询（ask）\n\n**模型连接说明**\n\n* 默认本地模型：llama3.1:8b\n* Ollama 地址：http://127.0.0.1:11434\n\n因此，可以总结如下：\n\n* 本地RAG的执行流程主要包括健康检查、配置知识目录、重建索引和发起查询四个步骤。\n* 数据加卸载能力允许添加和移除多个知识目录，并建议每次变更后重建索引。\n\n注意：上下文中没有提到具体的知识库加载和卸载能力，因此我只能根据提供的信息进行总结。",
    "sources": [
